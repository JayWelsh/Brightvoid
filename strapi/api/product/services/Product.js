...
module.exports = {
  ...
  fetchAll: (params, next, { populate } = {}) => {
    ...
    const hook = strapi.hook[Product.orm];
    // Generate stages.
    const populateStage = hook.load().generateLookupStage(Product, { whitelistedPopulate: populate }); // Nested-Population
    const matchStage = hook.load().generateMatchStage(Product, filters); // Nested relation filter
    const aggregateStages = mergeStages(populateStage, matchStage);

    const result = Product.aggregate(aggregateStages)
      .skip(filters.start)
      .limit(filters.limit);
