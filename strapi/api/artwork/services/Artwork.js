...
module.exports = {
  ...
  fetchAll: (params, next, { populate } = {}) => {
    ...
    const hook = strapi.hook[Artwork.orm];
    // Generate stages.
    const populateStage = hook.load().generateLookupStage(Artwork, { whitelistedPopulate: populate }); // Nested-Population
    const matchStage = hook.load().generateMatchStage(Artwork, filters); // Nested relation filter
    const aggregateStages = mergeStages(populateStage, matchStage);

    const result = Artwork.aggregate(aggregateStages)
      .skip(filters.start)
      .limit(filters.limit);
  ...
  edit: async (params, values) => {
    ...
    const entry = await Artwork.update(params, data, { multi: true });
